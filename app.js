...
function renderDashboard(){
  ...
  if(greetEl) greetEl.textContent = timeGreeting()+', '+t.currentUser.name.split(' ')[0];
...
function avatarColorClass(name){
  ...
  return 'av-' + (Math.abs(h) % 10);
}

function coloredAvatar(name, extraStyle){
  const cls = avatarColorClass(name);
  return '<span class="avatar '+cls+'"'+(extraStyle?' style="'+extraStyle+'"':'')+'>'+initials(name)+'</span>';
}
...
function runGlobalSearch(q){
  ...
  gsResults = [];
  if(!ql){ renderGSResults([]); return; }
  const t = currentTenant();
  t.employees.forEach(e=>{
    if((e.name+' '+e.title+' '+e.department).toLowerCase().includes(ql))
      gsResults.push({ icon:'👤', label:e.name, sub:e.title+' · '+e.department, module:'Employees', action:()=>{ goToPage('employees'); renderEmployeesPage(false); setTimeout(()=>openDrawer(e.id),100); } });
  });
  (t.tickets||[]).forEach(x=>{
    if((x.subject+' '+x.category).toLowerCase().includes(ql))
      gsResults.push({ icon:'🎫', label:x.subject, sub:x.category+' · '+x.status, module:'Service Desk', action:()=>{ goToPage('servicedesk'); renderServiceDeskPage(); } });
  });
  (t.announcements||[]).forEach(x=>{
    if((x.title+' '+(x.body||'')).toLowerCase().includes(ql))
      gsResults.push({ icon:'📢', label:x.title, sub:x.category+' · '+x.when, module:'Announcements', action:()=>{ goToPage('announcements'); renderAnnouncementsPage(); } });
  });
  (t.knowledgeArticles||[]).forEach(x=>{
    if((x.title+' '+x.content).toLowerCase().includes(ql))
      gsResults.push({ icon:'📚', label:x.title, sub:x.category, module:'Knowledge Base', action:()=>{ goToPage('knowledge'); currentKBSearch=''; renderKnowledgePage(); setTimeout(()=>expandKBArticle(x.id),100); } });
  });
  (t.courses||[]).forEach(x=>{
    if(x.title.toLowerCase().includes(ql))
      gsResults.push({ icon:x.emoji||'📋', label:x.title, sub:x.category+' · '+x.durationHrs+'h', module:'Learning', action:()=>{ goToPage('learning'); renderLearningPage(); } });
  });
  (t.openings||[]).filter(o=>o.status==='Open').forEach(x=>{
    if((x.title+' '+x.department).toLowerCase().includes(ql))
      gsResults.push({ icon:'💼', label:x.title, sub:x.department+' · '+x.location, module:'Recruitment', action:()=>{ goToPage('recruitment'); renderRecruitPage(); } });
  });
  renderGSResults(gsResults.slice(0,10));
}

function renderGSResults(results){
  const el = document.getElementById('gs-results');
  gsFocusIdx = -1;
  if(results.length===0){
    const q = document.getElementById('gs-input').value.trim();
    el.innerHTML = '<div class="gs-empty">'+(q?'No results for "'+q+'"':'Start typing to search across the platform')+'</div>';
    return;
  }
  el.innerHTML = results.map((r,i)=>
    '<div class="gs-result" data-gs-idx="'+i+'">'+
    '<div class="gr-icon">'+r.icon+'</div>'+
    '<div class="gr-meta"><div class="gr-label">'+r.label+'</div><div class="gr-sub">'+r.sub+'</div></div>'+
    '<span class="gr-module">'+r.module+'</span></div>'
  ).join('');
  el.querySelectorAll('.gs-result').forEach(item=>{
    item.addEventListener('click',()=>{ const r=gsResults[Number(item.dataset.gsIdx)]; if(r){ r.action(); closeGlobalSearch(); } });
    item.addEventListener('mouseenter',()=>{ document.querySelectorAll('.gs-result.focused').forEach(x=>x.classList.remove('focused')); item.classList.add('focused'); gsFocusIdx=Number(item.dataset.gsIdx); });
  });
}
...
function initScrollToTop(){
  ...
  if(!main||!btn) return;
  main.addEventListener('scroll', ()=>{ btn.classList.toggle('visible', main.scrollTop > 300); });
...
function renderQuickActions(){
  ...
  if(!row) return;
  const actions = [
    { icon:'➕', label:'Add employee', fn:()=> openAddModal() },
    { icon:'🗓️', label:'Approve leave', fn:()=>{ goToPage('leave'); renderLeavePage(); } },
    { icon:'💰', label:'Run payroll', fn:()=>{ goToPage('payroll'); renderPayrollPage(); } },
    { icon:'📋', label:'New ticket', fn:()=>{ goToPage('servicedesk'); renderServiceDeskPage(); setTimeout(openTicketModal,100); } },
    { icon:'🎯', label:'Add goal', fn:()=>{ goToPage('performance'); renderPerformancePage(); setTimeout(openGoalModal,100); } },
    { icon:'📢', label:'Announce', fn:()=>{ goToPage('announcements'); renderAnnouncementsPage(); setTimeout(openAnnouncementModal,100); } },
  ];
  row.innerHTML = actions.map((a,i)=>
    '<button class="quick-action-btn" data-qa="'+i+'"><span class="qa-icon">'+a.icon+'</span>'+a.label+'</button>'
  ).join('');
  row.querySelectorAll('[data-qa]').forEach(btn=>{
    btn.addEventListener('click', ()=> actions[Number(btn.dataset.qa)].fn());
  });
}
...
function renderCelebrations(){
  const t = currentTenant();
  const el = document.getElementById('celebrations-list');
  if(!el) return;
  const today = new Date();
  const items = [];
  t.employees.forEach(e=>{
    if(!e.hireDate) return;
    const d = new Date(e.hireDate+'T00:00:00');
    const years = today.getFullYear() - d.getFullYear();
    const sameDay = d.getMonth()===today.getMonth() && d.getDate()===today.getDate();
    const within7 = d.getMonth()===today.getMonth() && d.getDate()>today.getDate() && d.getDate()-today.getDate()<=7;
    if(sameDay && years>0) items.push({ name:e.name, event:'Work anniversary', detail:years+'yr', emoji:'🎉', badge:'Today' });
    else if(within7 && years>0) items.push({ name:e.name, event:'Work anniversary', detail:years+'yr', emoji:'🎂', badge:'In '+(d.getDate()-today.getDate())+' days' });
  });
  const hint = document.getElementById('celebr-hint');
  if(hint) hint.textContent = items.length ? items.length+' upcoming' : 'Birthdays & anniversaries';
  if(items.length===0){ el.innerHTML='<div class="empty-inline">No anniversaries in the next 7 days.</div>'; return; }
  el.innerHTML = items.map(it=>
    '<div class="celebration-item">'+
    '<span class="cel-emoji">'+it.emoji+'</span>'+
    '<span class="cel-meta"><span class="cm-name">'+it.name+'</span><br><span class="cm-event">'+it.event+' · '+it.detail+'</span></span>'+
    '<span class="cel-badge">'+it.badge+'</span></div>'
  ).join('');
}

/* ===================================================================== TEAM AT A GLANCE */

function renderTeamGlance(){
  const t = currentTenant();
  const el = document.getElementById('team-glance-list');
  if(!el) return;
  const onLeave = t.employees.filter(e=>e.status==='On leave').length;
  const active = t.employees.filter(e=>e.status==='Active').length;
  const probation = t.employees.filter(e=>e.status==='Probation').length;
  const pending = (t.leaveRequests||[]).filter(r=>r.status==='Pending').length;
  const items = [
    { label:'Working today', value:active, icon:'🟢' },
    { label:'On leave', value:onLeave, icon:'🏖️' },
    { label:'On probation', value:probation, icon:'📋' },
    { label:'Leave requests pending', value:pending, icon:'⏳' },
  ];
  el.innerHTML = items.map(it=>
    '<div class="glance-row"><span style="font-size:16px;">'+it.icon+'</span><span>'+it.label+'</span><span class="gr-badge">'+it.value+'</span></div>'
  ).join('');
}
...
function renderTeamLeaveCalendar(){
  const el = document.getElementById('team-leave-calendar');
  if(!el) return;
  const t = currentTenant();
  const today = new Date(); today.setHours(0,0,0,0);
  const weekStart = new Date(today); weekStart.setDate(today.getDate()-today.getDay()+1);
  const days = Array.from({length:7},(_,i)=>{ const d=new Date(weekStart); d.setDate(weekStart.getDate()+i); return d; });
  const approved = (t.leaveRequests||[]).filter(r=>r.status==='Approved');
  const employees = t.employees.slice(0,8);
  let html = '<div class="leave-calendar">';
  html += '<div class="lc-head">Name</div>';
  days.forEach(d=>{
    const isToday = d.getTime()===today.getTime();
    const isWkd = d.getDay()===0||d.getDay()===6;
    html += '<div class="lc-head'+(isWkd?' weekend':isToday?' active-day':'')+'">'+d.toLocaleDateString('en-GB',{weekday:'short'})+' '+d.getDate()+'</div>';
  });
  employees.forEach(e=>{
    html += '<div class="lc-name"><span class="avatar '+avatarColorClass(e.name)+'" style="width:20px;height:20px;font-size:8px;flex-shrink:0;">'+initials(e.name)+'</span>'+e.name.split(' ')[0]+'</div>';
    days.forEach(d=>{
      const isWkd = d.getDay()===0||d.getDay()===6;
      const onL = approved.some(r=>r.employeeId===e.id && dateOnly(r.startDate)<=d && dateOnly(r.endDate)>=d);
      const isT = d.getTime()===today.getTime();
      html += '<div class="lc-cell'+(isWkd?' weekend':onL&&isT?' on-leave-today':onL?' on-leave':'')+'" title="'+(onL?e.name+' on leave':'')+'"></div>';
    });
  });
  html += '</div>';
  if(employees.length < t.employees.length)
    html += '<div style="font-size:11.5px;color:var(--ink-faint);padding:6px 4px;">Showing first 8 of '+t.employees.length+' employees</div>';
  el.innerHTML = html;
}

/* ===================================================================== 360 REVIEWS */

let currentReviewType = 'manager';

function renderReviewTypePanel(type){
  currentReviewType = type;
  document.querySelectorAll('.review-type-tab').forEach(b=>b.classList.toggle('active',b.dataset.reviewType===type));
  const t = currentTenant();
  const listEl = document.getElementById('review-list');
  if(!listEl) return;
  if(type==='manager'){ renderReviewList(); return; }
  if(type==='self'){
    listEl.innerHTML = t.employees.map(e=>{
      const sr = t.selfReviews && t.selfReviews[e.id];
      const cls = sr?'status-active':'status-probation';
      return '<div class="review-row">'+
        '<span class="rvmeta"><span class="rvname">'+e.name+'</span><span class="rvdetail">'+e.title+'</span></span>'+
        '<span class="status-pill '+cls+'"><span class="dot"></span>'+(sr?'Submitted':'Not started')+'</span>'+
        (sr?'<span style="font-size:12px;color:var(--ink-faint);">★ '+sr.rating+'/5</span>':
          '<button class="btn btn-quiet" data-self-review="'+e.id+'" style="font-size:11.5px;padding:6px 10px;">Submit</button>')+
        '</div>';
    }).join('');
    listEl.querySelectorAll('[data-self-review]').forEach(btn=>btn.addEventListener('click',()=>submitSelfReview(btn.dataset.selfReview)));
  } else {
    listEl.innerHTML =
      '<div style="background:var(--paper-sunken);padding:20px;border-radius:var(--r-md);">'+
      '<p style="font-size:13.5px;margin-bottom:14px;">Select a colleague and give them a peer rating.</p>'+
      '<div class="field"><label>Who are you reviewing?</label><select id="peer-nominee-select">'+
        t.employees.map(e=>'<option value="'+e.id+'">'+e.name+'</option>').join('')+'</select></div>'+
      '<div class="star-picker" id="peer-star-picker">'+[1,2,3,4,5].map(n=>'<button data-peer-star="'+n+'">★</button>').join('')+'</div>'+
      '<div class="field" style="margin-top:10px;"><label>Peer feedback</label><textarea id="peer-feedback" rows="2" placeholder="What does this person do well?"></textarea></div>'+
      '<button class="btn btn-primary" id="submit-peer-btn" style="margin-top:8px;">Submit peer review</button></div>';
    let peerStars=0;
    listEl.querySelectorAll('[data-peer-star]').forEach((btn,i)=>{
      btn.addEventListener('click',()=>{ peerStars=Number(btn.dataset.peerStar); listEl.querySelectorAll('[data-peer-star]').forEach((b,j)=>b.classList.toggle('lit',j<peerStars)); });
    });
    document.getElementById('submit-peer-btn').addEventListener('click',()=>{
      const id=document.getElementById('peer-nominee-select').value;
      const feedback=document.getElementById('peer-feedback').value.trim();
      if(!peerStars){ showToast('Choose a rating first'); return; }
      if(!t.peerReviews) t.peerReviews={};
      if(!t.peerReviews[id]) t.peerReviews[id]=[];
      t.peerReviews[id].push({rating:peerStars,feedback,when:new Date().toLocaleDateString('en-GB')});
      const e=findEmployee(id);
      logActivity(t,'Peer review submitted for '+(e?e.name:'employee'));
      persistState();
      peerStars=0; document.getElementById('peer-feedback').value='';
      listEl.querySelectorAll('[data-peer-star]').forEach(b=>b.classList.remove('lit'));
      showToast('Peer review submitted for '+(e?e.name:'colleague'));
    });
  }
}

function submitSelfReview(empId){
  const t=currentTenant(); const e=findEmployee(empId); if(!e) return;
  if(!t.selfReviews) t.selfReviews={};
  t.selfReviews[empId]={rating:Math.floor(Math.random()*2)+3,comment:'Self-review submitted.',when:new Date().toLocaleDateString('en-GB')};
  logActivity(t,e.name+' submitted a self-review');
  persistState();
  showToast('Self-review submitted for '+e.name);
  renderReviewTypePanel('self');
}
...
function handleCSVImport(file){
  ...
  reader.onload=(ev)=>{
    ...
    const dateIdx=headers.findIndex(h=>h.includes('date')||h.includes('start'));
    if(nameIdx===-1){ showToast('CSV must have a "name" column'); return; }
    const t=currentTenant(); let added=0;
    lines.slice(1).forEach(line=>{
      const cols=line.split(',').map(c=>c.trim().replace(/"/g,''));
      const name=cols[nameIdx]; if(!name) return;
      const title=titleIdx>-1?cols[titleIdx]:'Employee';
      const department=deptIdx>-1&&t.departments.includes(cols[deptIdx])?cols[deptIdx]:t.departments[0];
      const salary=salaryIdx>-1?Number(cols[salaryIdx])||1200000:1200000;
      const hireDate=dateIdx>-1?cols[dateIdx]:new Date().toISOString().slice(0,10);
      const newId=t.key.toUpperCase()+'-'+String(1000+t.employees.length+added).slice(-4);
      t.employees.push({id:newId,name,title,department,salary,hireDate,status:'Active',history:[]});
      added++;
    });
    logActivity(currentTenant(),added+' employees imported via CSV');
    renderEmployeeTable(); renderDeptChips(); renderDashboard(); persistState();
    showToast(added+' employee'+(added===1?'':'s')+' imported from CSV');
  };
...
function renderLineChart(containerId, pairs, formatValue){
  const el=document.getElementById(containerId);
  if(!el||!pairs||pairs.length<2){ if(el) renderBarRows(containerId,pairs,formatValue); return; }
  const vals=pairs.map(p=>Number(p[1])||0);
  const labels=pairs.map(p=>p[0]);
  const t=currentTenant();
  const barW=40,gap=20,padL=10,padR=10,padT=20,padB=50,chartH=120;
  const totalW=pairs.length*(barW+gap)-gap+padL+padR;
  const maxV=Math.max(...vals,1),minV=Math.min(...vals,0),range=maxV-minV||1;
  const pts=vals.map((v,i)=>[(padL+i*(barW+gap)+barW/2),(padT+chartH-Math.round((v-minV)/range*chartH))]);
  const pathD='M'+pts.map(p=>p[0]+','+p[1]).join(' L');
  let dots='',xlabels='',yvals='';
  pts.forEach(([x,y],i)=>{
    dots+='<circle cx="'+x+'" cy="'+y+'" r="4" fill="'+t.accent+'"/>';
    xlabels+='<text class="svg-label" x="'+x+'" y="'+(padT+chartH+18)+'" text-anchor="middle">'+labels[i]+'</text>';
    yvals+='<text class="svg-value-label" x="'+x+'" y="'+(y-8)+'" text-anchor="middle">'+(formatValue?formatValue(vals[i]):vals[i])+'</text>';
  });
  const svgH=padT+chartH+padB;
  el.innerHTML='<div class="svg-chart"><svg width="'+totalW+'" height="'+svgH+'" viewBox="0 0 '+totalW+' '+svgH+'"><path class="svg-line" d="'+pathD+'" stroke="'+t.accent+'"/>'+dots+yvals+xlabels+'</svg></div>';
}
...
function resetSessionTimer(){
  ...
  _sessionTimer=setTimeout(()=>{
    ...
    const tick=setInterval(()=>{
      ...
      if(el) el.textContent='0:'+(secs<10?'0':'')+secs;
      if(secs<=0){
        clearInterval(tick);
        if(warn) warn.classList.remove('visible');
        document.getElementById('shell').style.display='none';
        document.getElementById('login-screen').style.display='flex';
        document.getElementById('login-hint').textContent='You were signed out due to inactivity.';
      }
    },1000);
...
document.querySelectorAll('.nav-item').forEach(item=>{
  item.addEventListener('click', ()=>{
    if(item.dataset.implemented === 'true'){
      ...
      updateActiveNav(item.dataset.page);
      closeMobileSidebar();
      if(item.dataset.page === 'employees') renderEmployeesPage(false);
      if(item.dataset.page === 'leave'){ renderLeavePage(); renderTeamLeaveCalendar(); }
      ...
      if(item.dataset.page === 'succession') renderSuccessionPage();
      if(item.dataset.page === 'workforce') renderWorkforcePage();
      if(item.dataset.page === 'compensation') renderCompensationPage();
      if(item.dataset.page === 'executive') renderExecutivePage();
      if(item.dataset.page === 'signatures') renderSignaturesPage();
    } else {
...
/* Succession */
document.getElementById('add-successor-btn').addEventListener('click', openSuccessorModal);
document.getElementById('successor-modal-close').addEventListener('click', closeSuccessorModal);
document.getElementById('successor-modal-cancel').addEventListener('click', closeSuccessorModal);
document.getElementById('successor-modal-submit').addEventListener('click', submitSuccessor);
document.getElementById('successor-modal-overlay').addEventListener('click', ev=>{ if(ev.target.id==='successor-modal-overlay') closeSuccessorModal(); });

/* Workforce */
document.getElementById('add-headcount-btn').addEventListener('click', openHeadcountModal);
document.getElementById('headcount-modal-close').addEventListener('click', closeHeadcountModal);
document.getElementById('headcount-modal-cancel').addEventListener('click', closeHeadcountModal);
document.getElementById('headcount-modal-submit').addEventListener('click', submitHeadcountPlan);
document.getElementById('headcount-modal-overlay').addEventListener('click', ev=>{ if(ev.target.id==='headcount-modal-overlay') closeHeadcountModal(); });

/* Compensation */
document.getElementById('run-merit-btn').addEventListener('click', runMeritCycle);

/* Executive */
document.getElementById('exec-print-btn').addEventListener('click', ()=>{
  document.getElementById('print-area').innerHTML = document.getElementById('page-executive').innerHTML;
  window.print();
});

/* Signatures */
document.getElementById('new-signature-request-btn').addEventListener('click', openSigRequestModal);
document.getElementById('sig-request-modal-close').addEventListener('click', closeSigRequestModal);
document.getElementById('sig-request-modal-cancel').addEventListener('click', closeSigRequestModal);
document.getElementById('sig-request-modal-submit').addEventListener('click', submitSigRequest);
document.getElementById('sig-request-modal-overlay').addEventListener('click', ev=>{ if(ev.target.id==='sig-request-modal-overlay') closeSigRequestModal(); });
document.getElementById('sig-sign-modal-close').addEventListener('click', closeSignModal);
document.getElementById('sig-sign-modal-cancel').addEventListener('click', closeSignModal);
document.getElementById('sig-sign-modal-submit').addEventListener('click', submitSign);
document.getElementById('sig-sign-modal-overlay').addEventListener('click', ev=>{ if(ev.target.id==='sig-sign-modal-overlay') closeSignModal(); });
...
document.getElementById('bell-btn').addEventListener('click', ()=>{ goToPage('notifications'); updateActiveNav('notifications'); renderNotifications(); });
document.getElementById('go-shifts-btn').addEventListener('click', ()=>{ goToPage('shifts'); updateActiveNav('shifts'); renderShifts(); });
...
/* Global search */
document.getElementById('gs-trigger-btn').addEventListener('click', openGlobalSearch);
document.getElementById('global-search-overlay').addEventListener('click', ev=>{ if(ev.target.id==='global-search-overlay') closeGlobalSearch(); });
document.getElementById('gs-input').addEventListener('input', ev=>{ runGlobalSearch(ev.target.value); });
document.getElementById('gs-input').addEventListener('keydown', ev=>{
  const items = document.querySelectorAll('.gs-result');
  if(ev.key==='ArrowDown'){ ev.preventDefault(); gsFocusIdx=Math.min(gsFocusIdx+1,items.length-1); items.forEach((el,i)=>el.classList.toggle('focused',i===gsFocusIdx)); }
  else if(ev.key==='ArrowUp'){ ev.preventDefault(); gsFocusIdx=Math.max(gsFocusIdx-1,0); items.forEach((el,i)=>el.classList.toggle('focused',i===gsFocusIdx)); }
  else if(ev.key==='Enter' && gsFocusIdx>=0 && gsResults[gsFocusIdx]){ gsResults[gsFocusIdx].action(); closeGlobalSearch(); }
});
...
/* Review type tabs */
document.querySelectorAll('.review-type-tab').forEach(btn=>{
  btn.addEventListener('click', ()=> renderReviewTypePanel(btn.dataset.reviewType));
});

/* CSV import */
document.getElementById('csv-import-input').addEventListener('change', ev=>{ handleCSVImport(ev.target.files[0]); ev.target.value=''; });
...
document.getElementById('session-stay-btn').addEventListener('click', ()=>{
  ...
  resetSessionTimer();
  showToast('Session extended — you are still signed in');
});

/* GDPR notice */
document.getElementById('gdpr-accept-btn').addEventListener('click', ()=>{
  document.getElementById('gdpr-notice').style.display='none';
  try{ localStorage.setItem('vantage_gdpr_accepted','1'); } catch(e){}
});

/* Activity resets session timer on every interaction */
...
document.addEventListener('keydown', (ev)=>{
  if((ev.metaKey||ev.ctrlKey) && ev.key==='k'){ ev.preventDefault(); openGlobalSearch(); return; }
  if(ev.key === 'Escape'){
    closeGlobalSearch();
    closeDrawer();
    closeAddModal();
...
if(todayEl) todayEl.textContent = new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
...
// signIn function wires the shell display
document.getElementById('shell').style.display = 'none';
document.getElementById('login-screen').style.display = 'flex';
document.getElementById('login-mark').style.background = currentTenant().accent;
